...
class GenerateImageTool(ToolInterface):
    ...
    def description(self) -> str:
        return ("Generates an image from a text prompt. Specify aspect ratio if needed. "
        ...
        try:
            ...
            if not image_data and settings.REPLICATE_API_TOKEN:
                try:
                    ...
                    image_url = await replicate_client.generate_image(prompt, aspect_ratio=aspect_ratio)
...
class GenerateVideoTool(ToolInterface):
    ...
    @property
    def description(self) -> str:
        return ("Generates a short video clip from a text prompt, optionally using an input image as a reference. "
        ...
        try:
            ...
            if input_s3_image_url and hasattr(context, 's3_service'):
                try:
                    ...
                    input_image_bytes = await context.s3_service.download_file_data(input_s3_image_url)
