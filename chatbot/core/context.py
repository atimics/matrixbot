...
class ContextManager:
    ...
    async def _update_world_state_in_context(self, channel_id: str):
        ...
        if current_world_state != context.world_state:
            ...
            context.system_prompt = await self._build_world_state_system_prompt(current_world_state)
            context.last_update = time.time()
            
            logger.debug(f"ContextManager: Updated world state in context for {channel_id}")
    
    async def _build_world_state_system_prompt(self, world_state: Dict[str, Any]) -> str:
...
Base your decisions on the current world state and user messages."""
    ...
    async def add_user_message(self, channel_id: str, message: Dict[str, Any]):
        ...
        await self._store_state_change(StateChangeBlock(
            timestamp=time.time(),
    ...
    async def add_tool_result(self, channel_id: str, tool_name: str, result: Dict[str, Any]):
    ...
    async def add_world_state_update(self, update_type: str, update_data: Dict[str, Any]):
    ...
    async def _store_state_change_file(self, state_change: StateChangeBlock):
        ...
        try:
            filename = f"state_change_{state_change.timestamp}_{state_change.source}.json"
    ...
    async def get_conversation_messages(self, channel_id: str, include_system: bool = True) -> List[Dict[str, Any]]:
        ...
        all_messages.extend([{**msg, "role": "assistant"} for msg in context.assistant_messages])
    ...
    async def get_state_changes(self, 
                               channel_id: Optional[str] = None,
                               change_type: Optional[str] = None,
                               since_timestamp: Optional[float] = None,
                               limit: int = 100) -> List[StateChangeBlock]:
    ...
    async def export_state_changes_for_training(self, 
                                               output_path: str,
        ...
        if format == "jsonl":
            ...
            with open(output_file, 'w') as f:
                json.dump([asdict(change) for change in self.state_changes], f, indent=2)
        else:
            raise ValueError(f"Unsupported format: {format}")
        
        logger.info(f"ContextManager: Exported {len(self.state_changes)} state changes to {output_file}")
