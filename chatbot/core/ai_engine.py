...
# These methods can be removed once the rest of the application is updated
# to use the new `decide_actions` interface.

class AIEngineLegacy(AIEngine):
    """Extends AIEngine with methods for backward compatibility."""
    ...
    async def make_decision(self, world_state: Dict[str, Any], cycle_id: str) -> Dict[str, Any]:
        """Legacy decision-making method."""
        logger.warning("Using legacy method 'make_decision'. Please update to 'decide_actions'.")
        
        context = self._extract_context_from_world_state(world_state)
        context["cycle_id"] = cycle_id

        decision = await self.decide_actions(context)
        
        # Translate to old format
        selected_actions = decision.get("selected_actions", [])
        if len(selected_actions) > self.max_actions_per_cycle:
            logger.warning(f"AI planned {len(selected_actions)} actions; limiting to {self.max_actions_per_cycle}.")
            selected_actions = selected_actions[:self.max_actions_per_cycle]

        return {
            "selected_actions": selected_actions,
            "reasoning": decision.get("reasoning", ""),
            "observations": decision.get("reasoning", ""), # Map reasoning to old observations key
            "thought": decision.get("reasoning", ""), # Map reasoning to old thought key
            "cycle_id": cycle_id,
        }

    def _extract_context_from_world_state(self, world_state: Dict[str, Any]) -> Dict[str, Any]:
        """Extracts context from the potentially complex world_state object."""
        context = {}
        # Extract messages from various possible locations
        if "expanded_nodes" in world_state:
            messages = []
            for node_info in world_state["expanded_nodes"].values():
                node_data = node_info.get("data", {})
                messages.extend(node_data.get("messages", []))
            if messages:
                messages.sort(key=lambda m: m.get("timestamp", 0))
                context["recent_messages"] = [
                    {"author": m.get("sender_username", "unknown"), "content": m.get("content", "")}
                    for m in messages[-15:]
                ]
        
        if "tools" in world_state:
            context["available_tools"] = [tool.get("name") for tool in world_state.get("tools", [])]

        if "current_processing_channel_id" in world_state:
             context["current_processing_channel_id"] = world_state["current_processing_channel_id"]
             
        # Extract trigger type
        primary_trigger = world_state.get("processing_context", {}).get("cycle_context", {}).get("primary_trigger_type")
        if primary_trigger:
            context["trigger_type"] = primary_trigger
