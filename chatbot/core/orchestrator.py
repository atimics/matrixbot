...
class EnhancedRateLimiter:
    ...
    def can_process_cycle(self, current_time: float) -> tuple[bool, float]:
        ...
        if self.config.enable_adaptive_limits:
            burst_cycles = len([t for t in self.cycle_history 
            ...
            if burst_cycles >= self.config.max_burst_cycles:
                ...
                self.cooldown_until = current_time + (base_interval * self.config.cooldown_multiplier)
                self.adaptive_multiplier = min(self.adaptive_multiplier * 1.2, 3.0)
                return False, self.cooldown_until - current_time
        
        return True, 0.0
        
    def can_execute_action(self, action_name: str, current_time: float) -> tuple[bool, str]:
        """
        ...
        if len(action_deque) >= limit:
            ...
            return False, f"Action rate limit exceeded: {len(action_deque)}/{limit} per hour. Wait {wait_time:.0f}s"
            
        return True, ""
        
    def can_send_to_channel(self, channel_id: str, channel_type: str, current_time: float) -> tuple[bool, str]:
        """
        ...
        if len(channel_deque) >= limit:
            ...
            return False, f"Channel rate limit exceeded: {len(channel_deque)}/{limit} per hour. Wait {wait_time:.0f}s"
...
class ContextAwareOrchestrator:
    ...
    async def _execute_action(self, action: Any) -> None:
        ...
        can_execute, rate_limit_reason = self.rate_limiter.can_execute_action(tool_name, current_time)
        if not can_execute:
            logger.warning(f"Rate limit prevents execution of {tool_name}: {rate_limit_reason}")
        ...
        if tool_name in ['SendMatrixMessageTool', 'SendMatrixReplyTool', 'SendFarcasterPostTool', 
            ...
            if not can_send:
                logger.warning(f"Channel rate limit prevents {tool_name} to {channel_id}: {channel_limit_reason}")
                await self.context_manager.add_tool_result(
                    channel_id,
                    tool_name,
                    {
    ...
    def log_rate_limit_status(self):
        ...
        logger.info(f"Rate Limit Status: Cycles {status['cycles_per_hour']}/{status['max_cycles_per_hour']}, "
        ...
        for action_name, action_status in status['action_limits'].items():
            ...
            if usage_percent > 70:  # Log if over 70% capacity
                logger.warning(f"Action {action_name} near limit: {action_status['used']}/{action_status['limit']} ({usage_percent:.1f}%)")
