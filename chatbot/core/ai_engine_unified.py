"""
Unified AI Decision Engine with multiple optimization levels.
Consolidates original, optimized, and balanced approaches into one clean implementation.
"""
...
# Import required data classes from original ai_engine
from .ai_engine import ActionPlan, DecisionResult
...
class UnifiedAIDecisionEngine(BaseAIDecisionEngine):
    """
    Unified AI Decision Engine supporting multiple optimization levels.
    ...
    def __init__(self, 
        ...
        self.logger.info(f"Initialized UnifiedAIDecisionEngine with {optimization_level} optimization")
...
Max 1 action. Use "wait" if no action needed."""
    ...
    async def make_decision(self, world_state: Dict[str, Any], cycle_id: str):
        ...
        """
        try:
            self.logger.info(f"UnifiedAIDecisionEngine: Starting decision cycle {cycle_id}")
            ...
            self.logger.info(
                f"UnifiedAIDecisionEngine: Cycle {cycle_id} complete - "
...
# Maintain original AIDecisionEngine class for backward compatibility
class AIDecisionEngine(UnifiedAIDecisionEngine):
    """Legacy AIDecisionEngine - now uses unified engine with ORIGINAL optimization."""
    
    def __init__(self, api_key: str, model: str = "openai/gpt-4o-mini", prompt_builder_instance=None):
        super().__init__(
            api_key=api_key,
            model=model,
            optimization_level=OptimizationLevel.ORIGINAL,
