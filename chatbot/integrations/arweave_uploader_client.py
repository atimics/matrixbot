...
class ArweaveUploaderClient:
    ...
    async def upload_data(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                tx_id = result.get("txid") or result.get("transaction_id") or result.get("id")
                
                if tx_id:
                    logger.info(f"ArweaveUploaderClient: Successfully uploaded data to Arweave: {tx_id}")
                    return tx_id
                else:
                    logger.error(f"ArweaveUploaderClient: Upload succeeded but no transaction ID in response: {result}")
                    return None
                    
        except httpx.HTTPStatusError as e:
            logger.error(f"ArweaveUploaderClient: HTTP error during upload: {e.response.status_code} - {e.response.text}")
            return None
    ...
    async def get_upload_status(self, tx_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"ArweaveUploaderClient: Failed to get status for {tx_id}: {e}")
            return None
