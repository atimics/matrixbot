        ...
        try:
            ...
            if images:
                if isinstance(images, str):
                    ...
                    logger.info(f"ReplicateClient: Randomly selected image from {len(images)} options")
            ...
            if self.lora_trigger_word:
                ...
                logger.info(f"ReplicateClient: Enhanced prompt with trigger word: {self.lora_trigger_word}")
            ...
            if lora_weights_url or self.default_lora_weights_url:
                payload["input"]["lora_weights"] = lora_weights_url or self.default_lora_weights_url
            ...
            logger.info(f"ReplicateClient: Starting generation with model {payload['version']}")
            ...
            async with httpx.AsyncClient(timeout=300.0) as client:
                ...
                if not response.is_success:
                    ...
                    logger.error(f"ReplicateClient: API error: {response.status_code} {error_text}")
                    return None
                ...
                while (prediction.get("status") in ["starting", "processing"] and 
                       time.time() - start_time < max_poll_time):
                    ...
                    if poll_response.is_success:
                        ...
                        logger.debug(f"ReplicateClient: Status {prediction.get('status')} after {time.time() - start_time:.1f}s")
                        
                        # Gradually increase poll interval (JavaScript pattern)
                        poll_interval = min(poll_interval * backoff_factor, max_interval)
                    else:
                        logger.warning(f"ReplicateClient: Poll request failed: {poll_response.status_code}")
                        break
                ...
                if prediction.get("status") == "succeeded":
                    ...
                    if isinstance(output, list) and len(output) > 0:
                        result_url = output[0]
                        logger.info(f"ReplicateClient: Generation succeeded: {result_url}")
                        return result_url
                    else:
                        logger.error("ReplicateClient: No output in successful prediction")
                        return None
                else:
                    status = prediction.get("status", "unknown")
                    error_msg = prediction.get("error", "No error details")
                    logger.error(f"ReplicateClient: Generation failed with status '{status}': {error_msg}")
                    return None
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.is_success:
                    ...
                    logger.error(f"ReplicateClient: Failed to get prediction status: {response.status_code}")
                    return None
