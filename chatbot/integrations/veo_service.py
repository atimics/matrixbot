...
class VeoService:
    ...
    def __init__(
        ...
        api_key: str,
        s3_service=None,
        default_model: str = "veo-2.0-generate-001"
    ):
        """
    ...
    async def generate_videos_from_images(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=600.0) as client:  # 10 min timeout for video
                ...
                if not response.is_success:
                    logger.error(f"VeoService: Failed to start generation: {response.status_code} {response.text}")
                    return []
                ...
                while not operation.get("done") and time.time() - start_time < max_poll_time:
                    ...
                    if poll_response.is_success:
                        ...
                        logger.info(f"VeoService: Video generation in progress... ({elapsed:.1f}s)")
                    else:
                        logger.warning(f"VeoService: Poll failed: {poll_response.status_code}")
                        break
                ...
                for i, video_info in enumerate(generated_videos):
                    ...
                    try:
                        ...
                        if download_response.is_success:
                            ...
                            logger.info(f"VeoService: Downloaded video ({len(video_bytes)} bytes)")
                            ...
                            if self.s3_service:
                                ...
                                try:
                                    logger.info(f"VeoService: Uploading video to S3: {temp_path}")
                                    s3_url = await self.s3_service.upload_image(temp_path)
                                    s3_urls.append(s3_url)
                                    logger.info(f"VeoService: Video uploaded to S3: {s3_url}")
                                finally:
                                    # Clean up temp file
                                    try:
                                        os.unlink(temp_path)
                                ...
                                logger.warning("VeoService: No S3 service available, skipping upload")
                                
                        else:
                            logger.warning(f"VeoService: Failed to download video: {download_response.status_code}")
