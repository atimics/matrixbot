...
def test_get_formatted_system_prompt_all_parts(mock_db):
...
@patch('prompt_constructor.database')
def test_get_formatted_system_prompt_no_channel_summary(mock_db):
    ...
    mock_db.get_user_memories.return_value = []
    prompt = get_formatted_system_prompt("TestBot", None, None, "dummy_db.sqlite", [])
...
@patch('prompt_constructor.database')
def test_get_formatted_system_prompt_no_global_summary(mock_db):
    mock_db.get_prompt.return_value = ("System prompt: {bot_identity_section}, {global_summary_section}, {user_memories_section}, {tool_states_section}", None)
    mock_db.get_latest_global_summary.return_value = None # No global summary
    mock_db.get_user_memories.return_value = []
    prompt = get_formatted_system_prompt("TestBot", "Channel summary", None, "dummy_db.sqlite", [])
    assert "TestBot" in prompt
...
@patch('prompt_constructor.database')
def test_get_formatted_system_prompt_no_summaries_no_bot_name(mock_db):
    mock_db.get_prompt.return_value = ("System prompt: {bot_identity_section}, {global_summary_section}, {user_memories_section}, {tool_states_section}", None)
...
@patch('prompt_constructor.database')
def test_get_formatted_system_prompt_only_bot_name(mock_db):
    mock_db.get_prompt.return_value = ("System prompt: {bot_identity_section}, {global_summary_section}, {user_memories_section}, {tool_states_section}", None)
    mock_db.get_latest_global_summary.return_value = None
    mock_db.get_user_memories.return_value = []
...
@patch('prompt_constructor.get_formatted_system_prompt') # Mock to avoid DB interaction for this part
def test_build_messages_for_ai_empty_inputs(mock_get_formatted_system_prompt):
    mock_get_formatted_system_prompt.return_value = "Mocked System Prompt"
    messages = build_messages_for_ai([], [], bot_display_name="TestBot", db_path="dummy.db")
    assert len(messages) == 1 # System prompt only
    assert messages[0]["role"] == "system"
    assert messages[0]["content"] == "Mocked System Prompt"

@patch('prompt_constructor.get_formatted_system_prompt')
def test_build_messages_for_ai_only_historical(mock_get_formatted_system_prompt):
    mock_get_formatted_system_prompt.return_value = "Mocked System Prompt"
    historical = [HistoricalMessage(role="user", content="Hello")]
    messages = build_messages_for_ai(historical, [], bot_display_name="TestBot", db_path="dummy.db")
    assert len(messages) == 2
    assert messages[1]["content"] == "Hello"

@patch('prompt_constructor.get_formatted_system_prompt')
def test_build_messages_for_ai_only_current_single(mock_get_formatted_system_prompt):
    mock_get_formatted_system_prompt.return_value = "Mocked System Prompt"
    current = [{"name": "@user:host", "content": "Hi there", "event_id": "$event1"}]
    messages = build_messages_for_ai([], current, bot_display_name="TestBot", db_path="dummy.db")
    assert len(messages) == 2
...
@patch('prompt_constructor.get_formatted_system_prompt')
def test_build_messages_for_ai_only_current_multiple(mock_get_formatted_system_prompt):
    mock_get_formatted_system_prompt.return_value = "Mocked System Prompt"
    current = [
